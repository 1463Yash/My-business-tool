...
export default function Vendors() {
  ...
  const handleDelete = async (id) => {
    const vendorsid = vendors.find((v) => v.id === id);
  ...
  const handlepaydues=async(id)=>{
    const vendorsdues=vendors.find(v=>v.id===id);
    const dues =vendorsdues.total_dues;
    if(dues<=0){
              ...
              {filteredVendors.map((v) => (
                <tr key={v.id}>
                  ...
                  <td>
                    <b>
                      ₹
                      {Number(v.total_dues).toLocaleString("en-IN", {
